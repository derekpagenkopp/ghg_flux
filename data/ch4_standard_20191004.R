...
fit_ch4_20191004 <- lm(ppm ~ area, data = ch4_standards %>% filter(date == "2019_10_04"))
...
ggplot_regression <- function (fit) {
  ...
  ggplot(fit$model, aes_string(x = names(fit$model)[1], y = names(fit$model)[2])) + 
    ...
    labs(title = "20191004 CH4 Standard Curve", caption = paste("Adj R2 = ",signif(summary(fit)$adj.r.squared, 5),
